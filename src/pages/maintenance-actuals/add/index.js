...
const AddMaintenanceActualPage = () => {
  ...
  const unitOptions = useMemo(() => units.map(toUnitSearchOption), [units])
  ...
  // Drop unit if it no longer belongs to the selected project
  useEffect(() => {
    if (!selectedUnitId) return
    if (unitOptions.some(o => String(o.value) === String(selectedUnitId))) return
    setValue('unitId', '')
  }, [selectedUnitId, unitOptions, setValue])
