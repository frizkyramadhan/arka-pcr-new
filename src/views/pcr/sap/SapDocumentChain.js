...
const DocCard = ({ type, item, highlighted, onOpen, subtitle, referenceChipLabel = 'BA Reference' }) => {
  if (!item) return <EmptyCell />

  const expiredLabel = getExpiredDisplayLabel(type, item)
  ...
  return (
    ...
    >
      ...
      <Typography variant='body2' sx={{ fontWeight: 700, lineHeight: 1.2 }}>
        {formatDocNumLabel(type, item.docNum)}
      </Typography>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mt: 0.75, flexWrap: 'wrap' }}>
        ...
        />
        {highlighted ? (
          <CustomChip rounded skin='light' size='small' color='primary' label={referenceChipLabel} />
        ) : null}
        {expiredLabel ? <ExpiredBadge label={expiredLabel} /> : null}
      </Box>
      {item.docDate ? (
...
const MrMiCombinedCard = ({ mr, highlighted, focusMiDocNum, onOpenMrMi, showMr, referenceChipLabel = 'BA Reference' }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant='body2' sx={{ fontWeight: 700, lineHeight: 1.2 }}>
          {formatDocNumLabel('mr', mr.docNum)}
        </Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mt: 0.75, flexWrap: 'wrap' }}>
          <CustomChip
            rounded
...
const ChainRow = ({ row, isHighlighted, onOpenDoc, onOpenMrMi, focusMiDocNum, referenceChipLabel }) => (
  <Box
    sx={{
      display: 'grid',
    ...
    <Box>
      ...
      {row.showPr ? (
        <DocCard
          type='pr'
          item={row.pr}
          highlighted={isHighlighted('pr', row.pr?.docNum)}
          onOpen={onOpenDoc}
          referenceChipLabel={referenceChipLabel}
        />
      ) : (
    ...
    <Box>
      ...
      {row.showPo ? (
        <DocCard
          type='po'
          item={row.po}
          highlighted={isHighlighted('po', row.po?.docNum)}
          onOpen={onOpenDoc}
          referenceChipLabel={referenceChipLabel}
        />
      ) : (
...
const SapDocumentChain = ({
  ...
  const highlightAnchors = useMemo(
    () => ({
      mr: normalizeDocNumValue(baReference?.mrNo ?? mrNo),
      pr: normalizeDocNumValue(baReference?.prNo ?? prNo),
      po: normalizeDocNumValue(baReference?.poNo ?? poNo)
    }),
    [baReference, mrNo, poNo, prNo]
  )

  const referenceLabels = useMemo(() => getReferenceLabels(referenceScope), [referenceScope])

  const hasWoSeed =
    hasDocNumValue(anchors.wo) || hasDocNumValue(anchors.woRemove) || hasDocNumValue(anchors.woInstall)
    ...
    const load = async () => {
      ...
      try {
        const data = await fetchSapDocumentChain({
          ...
          poNo: hasWoSeed ? undefined : anchors.po,
        ...
        if (!active) return
        setChain(null)
        setError(toFriendlySapErrorMessage(err, 'Failed to load SAP document chain.'))
      } finally {
  ...
  }, [anchors.mr, anchors.po, anchors.pr, anchors.wo, anchors.woInstall, anchors.woRemove, expanded, hasSeed, hasWoSeed])
  ...
  const isHighlighted = (type, docNum) => {
    ...
    return String(highlightAnchors[type] ?? '') === String(docNum)
  }
          ...
          <Box sx={{ minWidth: 0 }}>
            ...
            <Typography variant='caption' sx={{ color: 'text.secondary' }}>
              {expanded
                ...
                : 'Click to load SAP document chain (WO → MR/MI → PR → PO)'}
            </Typography>
          </Box>
        </Box>
        {expanded && chain ? (
        ...
        {!loading && !error && chain ? (
          rows.length === 0 ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              ...
              {rows.map((row, index) => (
                <ChainRow
                  key={`chain-row-${index}-${row.wo?.docNum ?? 'wo'}-${row.mr?.docNum ?? 'mr'}-${
                    row.pr?.docNum ?? 'pr'
                  }-${row.po?.docNum ?? 'po'}`}
                  row={row}
                  ...
                  referenceChipLabel={referenceLabels.chip}
                />
              ))}
            </Box>
          )
