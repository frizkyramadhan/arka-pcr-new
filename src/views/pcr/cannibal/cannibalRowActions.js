...
export const buildCannibalActions = (
  ...
  if (canEdit && ['DRAFT', 'REJECTED'].includes(row.statusBa)) {
    actions.push(
      { key: 'edit', label: 'Edit Plant Section', onClick: () => onAction('edit', row) },
      { key: 'submit-to-logistics', label: 'Send to Logistics', onClick: () => onAction('submit-to-logistics', row) }
    )
  }
  ...
  if (canEditExecution && row.statusBa === 'APPROVED') {
    actions.push({ key: 'execution', label: 'Update Record', onClick: () => onAction('execution', row) })
  }
  ...
  if (canEdit && ['SUBMITTED', 'REJECTED', 'PENDING_LOGISTICS'].includes(row.statusBa)) {
