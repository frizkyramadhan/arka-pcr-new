...
const LayoutAppBar = props => {
  ...
  const useAppBarBlur = appBarBlur && !isSemiDark

  const appBarBlurEffect = useAppBarBlur && {
    '&:after': {
      top: 0,
      left: 0,
      zIndex: -1,
  ...
  return (
    ...
    >
      <Toolbar
        ...
        sx={{
          ...(useAppBarBlur && { backdropFilter: 'blur(6px)' }),
          minHeight: theme => `${theme.mixins.toolbar.minHeight}px !important`,
          backgroundColor: theme => hexToRGBA(theme.palette.background.paper, useAppBarBlur ? 0.95 : 1),
          ...(skin === 'bordered'
