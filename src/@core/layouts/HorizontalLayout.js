...
import NavThemeProvider from 'src/@core/layouts/components/shared-components/NavThemeProvider'
...
const HorizontalLayout = props => {
  ...
  // ** Vars
  const { skin, appBar: appBarPosition, navHidden, appBarBlur, contentWidth } = settings
  ...
  const appBarSection = (
    <AppBar
      ...
      position={appBarPosition === 'fixed' ? 'sticky' : 'static'}
      sx={{
        alignItems: 'center',
        color: 'text.primary',
        justifyContent: 'center',
        ...(appBarPosition === 'static' && { zIndex: 13 }),
  ...
  return (
    <HorizontalLayoutWrapper className='layout-wrapper'>
      <MainContentWrapper className='layout-content-wrapper' sx={{ ...(contentHeightFixed && { maxHeight: '100vh' }) }}>
        ...
        <NavThemeProvider settings={settings}>{appBarSection}</NavThemeProvider>
        {/* Content */}
        <ContentWrapper
