...
/** Unit options scoped to the selected project (keeps edit selections visible). */
export function equipmentsForProject(equipments, projectCode, transfer, pairLines) {
  ...
  const ensureUnit = (side, line) => {
    const id = Number(side?.fleetUnitId)
    if (!id || byId.has(id)) return

    byId.set(id, {
      id,
      unit_no: line?.unitNo ?? line?.unit?.unitNo ?? line?.unit_no ?? `Unit #${id}`,
      project_code: projectCode
    })
  }

  ensureUnit(transfer?.remove, pairLines?.remove)
  ensureUnit(transfer?.install, pairLines?.install)
